...
> **状态（2026-09-22 更新）**：**阶段 1 已收官**（Task 1 retry 压缩已推送上云并实证生效、Task 2 额度哨兵、Task 3 双窗口 cron 已网页保存生效；**Task 4 验收记录已完成**——13:30 首班 09-22 13:37:57 落地 `no_change`，两班三证入主计划 §九「09-21/22 实施记录」；Task 10 文档收尾同步完成：规格状态行 + §8 实况、主计划 §十 补记 09-22 行）。
...
**推送前检查（🔴 09-22 血泪教训，务必逐条执行）**：
1. **必须先确认「当日批次已存在」**——`git -C /e/gitee/shadow fetch origin status && git show origin/status:<今日>.md` 有当日记录（或 `origin/main:data/daily-advice.json` 已有今天条目）。**当日批次未生成时推送会立刻起班并按当时（可能尚未开售的）行情把当天定格**：09-22 08:29 推 → 08:30 引擎跑完 → 零候选写成 `rest:true` 假休战 → 11:30 / 13:30 两班全空转，整天锁死。当日批次未生成就**等 11:30 之后再推**。
2. 避开 11:25~11:35 与 13:25~13:35（免与定时班并发）。
3. 当日批次已存在时，该班次会 `no_change` 空转，仅耗 2~4 分钟额度，安全。
